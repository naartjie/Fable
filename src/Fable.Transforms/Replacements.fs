        ...
        argTypes |> List.map (function
            | GenericParam g as t ->
                genArgs |> List.tryPick (fun (name2, t) ->
                    if g.Name = name2 then Some t else None)
    ...
    let objValue (k, v): Fable.MemberDecl =
        {
            ...
            Body = v
            UsedNames = Set.empty
            Info = FSharp2Fable.Convert.MemberInfo(isValue=true)
        }
...
let precompiledLib r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) (entityName, importPath) =
    let mangledName = getPrecompiledLibMangledName entityName i.CompiledName i.OverloadSuffix (Option.isNone thisArg)
    if i.IsModuleValue
...
let sets (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | _ ->
        let isStatic = Option.isNone thisArg
        let mangledName = Naming.buildNameWithoutSanitationFrom "FSharpSet" isStatic i.CompiledName i.OverloadSuffix
...
let types (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    let resolved =
        ...
        | Some(Value(TypeInfo exprType, exprRange) as thisArg) ->
            match exprType with
            | GenericParam g -> genericTypeInfoError g.Name |> addError com ctx.InlinePath exprRange
            | _ -> ()
            match i.CompiledName with
...
let tryBaseConstructor com ctx (ent: Entity) (argTypes: Type list) genArgs args =
