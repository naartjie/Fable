...
type Compiler =
    ...
    abstract GetOrAddEntity: entityFullName: string * (unit->AST.Fable.Entity) -> AST.Fable.Entity
    abstract GetEntity: entityFullName: string -> AST.Fable.Entity
    abstract GetOrAddInlineExpr: string * (unit->InlineExpr) -> InlineExpr
    abstract AddWatchDependency: file: string -> unit
    abstract AddLog: msg:string * severity: Severity * ?range: Fable.AST.SourceLocation
