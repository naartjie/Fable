...
type Convert() =
    static let mapToList (f: 'a -> 'b) (xs: IList<'a>) =
        let mutable li = []
        for i = xs.Count - 1 to 0 do
            li <- f xs.[i]::li
        li

    static let chooseToList (f: 'a -> 'b option) (xs: IList<'a>) =
        let mutable li = []
        for i = xs.Count - 1 to 0 do
            match f xs.[i] with
            | Some x -> li <- x::li
    ...
    static let makeType com t =
        TypeHelpers.makeType com Map.empty t

    static member GetOrAddEnt(com: Compiler, e: FSharpEntity) =
        com.GetOrAddEntity(Convert.EntFullName(e), (fun () -> Convert.Ent(com, e)))

    static member Field(com, fi: FSharpField) =
        ...
        Convert.Field
            (getFSharpFieldName fi,
             makeType com fi.FieldType,
             isMutable = fi.IsMutable,
             isStatic = fi.IsStatic,
             ?literalValue = fi.LiteralValue)

    static member Field(name, typ: Fable.Type, ?isMutable, ?isStatic, ?literalValue): Fable.Field =
        { Name = name
          FieldType = typ
    ...
    static member UnionCaseCompiledName(uci: FSharpUnionCase) =
        uci.Attributes
        |> Helpers.tryFindAtt Atts.compiledName
        |> Option.map (fun (att: FSharpAttribute) -> att.ConstructorArguments.[0] |> snd |> string)

    static member UnionCase(com, uci: FSharpUnionCase): Fable.UnionCase =
        { Name = uci.Name
          CompiledName = Convert.UnionCaseCompiledName uci
          UnionCaseFields =
              uci.UnionCaseFields
              |> mapToList (fun x -> Convert.Field(com, x)) }

    static member Att(att: FSharpAttribute): Fable.Attribute =
        { FullName = defaultArg att.AttributeType.TryFullName ""
          ConstructorArguments = att.ConstructorArguments |> mapToList snd }

    static member GenConstraint(com, c: FSharpGenericParameterConstraint) =
         if c.IsEnumConstraint then
             Some Fable.IsEnum // (makeType com c.EnumConstraintTarget)
//         elif c.IsDelegateConstraint then
//             let d = c.DelegateConstraintData
//             Fable.IsDelegate(makeType com d.DelegateTupledArgumentType, makeType com d.DelegateReturnType)
//         elif c.IsDefaultsToConstraint then
//             let d = c.DefaultsToConstraintData
//             Fable.DefaultsTo(d.DefaultsToPriority, makeType com d.DefaultsToTarget)
//         elif c.IsSimpleChoiceConstraint then
//             Fable.SimpleChoice(mapToList (makeType com) c.SimpleChoices)
         elif c.IsCoercesToConstraint then
             Some(Fable.CoercesTo(makeType com c.CoercesToTarget))
         elif c.IsSupportsNullConstraint then
             Some Fable.SupportsNull
         elif c.IsMemberConstraint then
             let d = c.MemberConstraintData
             Some(Fable.MayResolveMember { Name = d.MemberName
                                           IsStatic = d.MemberIsStatic
                                           Sources = mapToList (makeType com) d.MemberSources
                                           ArgTypes = mapToList (makeType com) d.MemberArgumentTypes })
         elif c.IsRequiresDefaultConstructorConstraint then
             Some Fable.RequiresDefaultConstructor
         elif c.IsNonNullableValueTypeConstraint then
             Some Fable.IsNonNullableStruct
         elif c.IsReferenceTypeConstraint then
             Some Fable.IsReferenceType
         elif c.IsComparisonConstraint then
             Some Fable.SupportsComparison
         elif c.IsEqualityConstraint then
             Some Fable.SupportsEquality
         else
             None

    static member GenParam(com, gen: FSharpGenericParameter): Fable.GenericParam =
        { Name = TypeHelpers.genParamName gen
          Constraints = gen.Constraints |> chooseToList (fun c -> Convert.GenConstraint(com, c)) }

    static member Param(com, p: FSharpParameter): Fable.Parameter =
        { Name = p.Name
          Type = makeType com p.Type
          IsOptional = p.IsOptionalArg
          Attributes =
              p.Attributes
              |> mapToList (fun x -> Convert.Att(x)) }

    static member DeclaredType(com, ent: FSharpEntity, genArgs: IList<FSharpType>): Fable.DeclaredType =
        { Definition = Convert.GetOrAddEnt(com, ent).FullName
          GenericArgs =
              genArgs
              |> mapToList (makeType com) }

    static member MemberFunctionOrValue(com, m: FSharpMemberOrFunctionOrValue): Fable.MemberFunctionOrValue =
        let info: Fable.MemberInfo =
            { Attributes =
                  m.Attributes
                  |> mapToList (fun x -> Convert.Att(x))
              ...
              HasSpread = Helpers.hasParamArray m
              IsPublic = Helpers.isPublicMember m
              ...
              IsSetter = m.IsPropertySetterMethod }

        { Info = info
          FullType = makeType com m.FullType
          DisplayName = Naming.removeGetSetPrefix m.DisplayName
          CompiledName = m.CompiledName
          FullName = m.FullName

          CurriedParameterGroups =
              m.CurriedParameterGroups
              |> mapToList (mapToList (fun p -> Convert.Param(com, p)))

          Attributes =
              m.Attributes
              |> mapToList (fun x -> Convert.Att(x))

          ReturnParameter = Convert.Param(com, m.ReturnParameter)
          IsCompilerGenerated = m.IsCompilerGenerated
          IsExtensionMember = m.IsExtensionMember
          IsExplicitInterfaceImplementation = m.IsExplicitInterfaceImplementation
          DeclaringEntity = m.DeclaringEntity |> Option.map (fun e -> Convert.GetOrAddEnt(com, e).FullName)
          ApparentEnclosingEntity = Convert.GetOrAddEnt(com, m.ApparentEnclosingEntity).FullName }

    static member EntIsPublic(ent: FSharpEntity) = not ent.Accessibility.IsPrivate

    static member EntSourcePath(ent: FSharpEntity) =
        ent.DeclarationLocation.FileName
        |> Path.normalizePathAndEnsureFsExtension

    static member EntFullName(ent: FSharpEntity) =
        match ent.TryFullName with
        | Some n -> n
        | None -> ent.CompiledName

    static member Ent(com, ent: FSharpEntity): Fable.Entity =
        { DisplayName = ent.DisplayName
          FullName = Convert.EntFullName ent
          SourcePath = Convert.EntSourcePath ent
          AssemblyPath = ent.Assembly.FileName
          ...
          BaseDeclaration =
              ...
              | Some baseType ->
                  ...
                  | Some (baseEntity, fullName) when fullName <> Some Types.object ->
                      Some(Convert.DeclaredType(com, baseEntity, baseType.GenericArguments))
                  | _ -> None
              | None -> None
          ...
          MembersFunctionsAndValues =
              ...
              |> mapToList (fun x -> Convert.MemberFunctionOrValue(com, x))

          AllInterfaces =
              ent.AllInterfaces
              |> Seq.chooseToList (fun ifc ->
                  if ifc.HasTypeDefinition
                  then Some(Convert.DeclaredType(com, ifc.TypeDefinition, ifc.GenericArguments))
                  else None)

          GenericParameters =
              ent.GenericParameters
              |> mapToList (fun x -> Convert.GenParam(com, x))
          ...
          UnionCases =
              ent.UnionCases
              |> mapToList (fun x -> Convert.UnionCase(com, x))

          IsPublic = Convert.EntIsPublic ent
          IsFSharpAbbreviation = ent.IsFSharpAbbreviation
          IsFSharpUnion = ent.IsFSharpUnion
          IsFSharpRecord = ent.IsFSharpRecord
          IsFSharpExceptionDeclaration = ent.IsFSharpExceptionDeclaration
          IsFSharpModule = ent.IsFSharpModule
          IsValueType = ent.IsValueType
          IsInterface = ent.IsInterface }

    static member MemberInfo(?attributes: IList<FSharpAttribute>,
                             ?hasSpread: bool,
                             ?isPublic: bool,
                             ?isStatic: bool,
                             ?isValue: bool,
        ...
        { Attributes =
              ...
              | Some atts -> atts |> mapToList Convert.Att
              | None -> []

          HasSpread = defaultArg hasSpread false
          IsPublic = defaultArg isPublic true
          IsStatic = defaultArg isStatic false
          IsValue = defaultArg isValue false
          IsMutable = defaultArg isMutable false
          IsGetter = defaultArg isGetter false
          IsSetter = defaultArg isSetter false
          IsEnumerator = defaultArg isEnumerator false
          IsMangled = defaultArg isMangled false }
...
type IFableCompiler =
    ...
    abstract TryReplace: Context * SourceLocation option * Fable.Type * info:Fable.ReplaceCallInfo * thisArg:Fable.Expr option * args:Fable.Expr list
     -> Fable.Expr option

    abstract InjectArgument: Context * SourceLocation option * genArgs:((string * Fable.Type) list) * Fable.Parameter
     -> Fable.Expr
...
module Helpers =
    let rec nonAbbreviatedType (t: FSharpType) =
        ...
        if t.IsAbbreviation
           && not (isSameType t t.AbbreviatedType) then
        ...
        // or can we just hardcode the names? We may need to do it anyway to fix #1962
            ...
            if t.GenericArguments.Count = abbr.GenericArguments.Count
            then t
            else abbr
        else
            t
    ...
    let getGenericArguments (t: FSharpType) =
        ...
        if t.IsGenericParameter then [||] :> IList<_> else (nonAbbreviatedType t).GenericArguments
    ...
    let cleanNameAsJsIdentifier (name: string) =
        if name = ".ctor" then "$ctor" else name.Replace('.', '_').Replace('`', '$')

    let getEntityDeclarationName (com: Compiler) (ent: Fable.Entity) =
        let entityName =
            getEntityMangledName com true ent
            |> cleanNameAsJsIdentifier

        (entityName, Naming.NoMemberPart)
        ||> Naming.sanitizeIdent (fun _ -> false)

    let private getMemberMangledName (com: Compiler) trimRootModule (memb: Fable.MemberFunctionOrValue) =
        if memb.IsExtensionMember then
            ...
            let entName =
                ...
                |> com.GetEntity
                |> getEntityMangledName com false
            ...
            | Some ent ->
                ...
                if ent.IsFSharpModule then
                    ...
                    if memb.Info.IsStatic
                    then entName, Naming.StaticMemberPart(memb.CompiledName, overloadSuffix)
                    else entName, Naming.InstanceMemberPart(memb.CompiledName, overloadSuffix)
    ...
    let getMemberDeclarationName (com: Compiler) (memb: Fable.MemberFunctionOrValue) =
        ...
        let sanitizedName =
            Naming.sanitizeIdent (fun _ -> false) name part
    ...
    let tryFindAtt fullName (atts: FSharpAttribute seq) =
        ...
        |> Seq.tryPick (fun att ->
            ...
            | Some fullName' -> if fullName = fullName' then Some att else None
            | None -> None)
    ...
    let hasAttribute attFullName (attributes: FSharpAttribute seq) =
        ...
        for att in attributes do
            found <-
                found
                || att.AttributeType.TryFullName = attFullName
        found
    ...
    let isPublicMember (memb: FSharpMemberOrFunctionOrValue) =
        if memb.IsCompilerGenerated then false else not memb.Accessibility.IsPrivate
    ...
    let unionCaseTag (ent: FSharpEntity) (unionCase: FSharpUnionCase) =
        ...
        with _ -> failwithf "Cannot find case %s in %s" unionCase.Name (getEntityFullName ent)
    ...
    let rec getAllInterfaceMembers (com: Compiler) (ent: Fable.Entity) =
        seq {
            yield! ent.MembersFunctionsAndValues
            for parent in ent.AllInterfaces do
                yield! com.GetEntity(parent.Definition)
                       |> getAllInterfaceMembers com
        }
    ...
    let hasParamArray (memb: FSharpMemberOrFunctionOrValue) =
        ...
        let hasParamArray (memb: FSharpMemberOrFunctionOrValue) =
            if memb.CurriedParameterGroups.Count <> 1 then
                ...
                args.Count > 0
                && args.[args.Count - 1].IsParamArrayArg
...
module Patterns =
        ...
        | (FSharpImplementationFileDeclaration.Entity (ent, subDecls)) :: restDecls when ent.IsNamespace ->
            ...
            ||> List.fold (fun acc decl ->
                    ...
                    | (Some subDecls), (FSharpImplementationFileDeclaration.Entity (ent, subDecls2)) ->
                        if ent.CompiledName = commonName then Some(subDecls @ subDecls2) else None
                    | _ -> None)
    ...
    let (|ForOf|_|) =
        ...
        | Let ((_, value),  // Coercion to seq
               Let ((_, Call (None, meth, _, [], [])), TryFinally (WhileLoop (_, Let ((ident, _), body)), _)))
        | Let ((_, Call (Some value, meth, _, [], [])), TryFinally (WhileLoop (_, Let ((ident, _), body)), _)) when
        ...
        | Let ((_, UnionCaseGet (value, typ, unionCase, field)), WhileLoop (_, Let ((ident, _), body))) when (getFsTypeFullName
                                                                                                                  typ) =
                                                                                                                 Types.list
                                                                                                             && unionCase.Name =
                                                                                                                 "op_ColonColon"
        ...
        | Let ((ident, UnionCaseGet (value, typ, unionCase, field)), WhileLoop (_, body)) when (getFsTypeFullName typ) =
                                                                                                   Types.list
                                                                                               && unionCase.Name =
                                                                                                   "op_ColonColon"
                                                                                               && field.Name = "Tail" ->
    ...
    let (|ByrefArgToTuple|_|) =
        ...
        | Let ((outArg1, (DefaultValue _ as def)),
               NewTuple (_, [ Call (callee, memb, ownerGenArgs, membGenArgs, callArgs); Value outArg3 ])) when List.isMultiple
                                                                                                                   callArgs
    ...
    let (|ByrefArgToTupleOptimizedIf|_|) =
        function
        | Let ((outArg1, (DefaultValue _ as def)),
               IfThenElse (Call (callee, memb, ownerGenArgs, membGenArgs, callArgs), thenExpr, elseExpr)) when List.isMultiple
                                                                                                                   callArgs
                                                                                                               && outArg1.IsCompilerGenerated ->
            match List.splitLast callArgs with
            | callArgs, AddressOf (Value outArg2) when outArg1 = outArg2 ->
    ...
    let (|ByrefArgToTupleOptimizedTree|_|) =
        ...
        | Let ((outArg1, (DefaultValue _ as def)),
               DecisionTree (IfThenElse (Call (callee, memb, ownerGenArgs, membGenArgs, callArgs), thenExpr, elseExpr),
                             targetsExpr)) when List.isMultiple callArgs
                                                && outArg1.IsCompilerGenerated ->
            match List.splitLast callArgs with
            | callArgs, AddressOf (Value outArg2) when outArg1 = outArg2 ->
                Some
    ...
    let (|ByrefArgToTupleOptimizedLet|_|) =
        function
        | Let ((outArg1, (DefaultValue _ as def)),
               Let ((arg_0, Call (callee, memb, ownerGenArgs, membGenArgs, callArgs)), restExpr)) when List.isMultiple
                                                                                                           callArgs
                                                                                                       && outArg1.IsCompilerGenerated ->
            match List.splitLast callArgs with
            | callArgs, AddressOf (Value outArg2) when outArg1 = outArg2 ->
                                       ...
                                       Lambda (_delegateArg0,
                                               Lambda (_delegateArg1,
                                                       Application (Value _callback',
                                                                    [],
                                                                    [ Value _delegateArg0'; Value _delegateArg1' ]))))) ]) when createEvent.FullName =
                                                                                                                                    Types.createEvent ->
            let eventName =
                addEvent.CompiledName.Replace("add_", "")
    ...
    let (|OptimizedOperator|_|) (com: Compiler) fsExpr =
        if com.Options.OptimizeFSharpAst then
            ...
            | Let ((var, Call (None, memb, _, membArgTypes, membArgs)),
                   DecisionTree (IfThenElse (_, _, IfThenElse (TypeTest (tt, Value vv), _, _)), _)) when var.FullName =
                                                                                                             "matchValue"
                                                                                                         && memb.FullName =
            ...
            | Call (Some expr, memb, _, [], [ Call (None, comp, [], [], []) ]) when memb.FullName.EndsWith
                                                                                        (".GetHashCode")
                                                                                    && comp.FullName =
            ...
            | Call (Some e1, memb, _, [], [ Coerce (t2, e2); Call (None, comp, [], [], []) ]) when memb.FullName.EndsWith
                                                                                                       (".Equals")
                                                                                                   && t2.HasTypeDefinition
                                                                                                   && t2.TypeDefinition.CompiledName =
                                                                                                       "obj"
                                                                                                   && comp.FullName =
    ...
    let (|NumberKind|_|) fullName =
        match Types.numberKind.TryGetValue(fullName) with
        | true, kind -> Some kind
        | false, _ -> None

    let (|OptionUnion|ListUnion|ErasedUnion|ErasedUnionCase|StringEnum|DiscriminatedUnion|) (NonAbbreviatedType typ: FSharpType,
    ...
    let (|ContainsAtt|_|) (fullName: string) (ent: FSharpEntity) = tryFindAtt fullName ent.Attributes
...
module TypeHelpers =
    ...
    let genParamName (genParam: FSharpGenericParameter) =
        if genParam.IsCompilerGenerated then genParam.Name + "$" else genParam.Name
    ...
    let makeGenArgs (com: Compiler) ctxTypeArgs (genArgs: IList<FSharpType>) =
        genArgs
        |> Seq.map (fun genArg ->
    ...
    let makeTypeFromDelegate com ctxTypeArgs (genArgs: IList<FSharpType>) (tdef: FSharpEntity) =
        ...
        let resolveType (t: FSharpType) =
            ...
            then Map.find (genParamName t.GenericParameter) genArgs
            else t

        let argTypes =
            argTypes
            |> Seq.map (resolveType >> makeType com ctxTypeArgs)
            |> Seq.toList
    ...
    let makeTypeFromDef (com: Compiler) ctxTypeArgs (genArgs: IList<FSharpType>) (tdef: FSharpEntity) =
        if tdef.IsArrayType then
            ...
            Fable.Enum (getOrAddEntity com tdef).FullName
        else
            ...
            | _ ->
                let ent = Convert.GetOrAddEnt(com, tdef)
                Fable.DeclaredType(ent.FullName, makeGenArgs com ctxTypeArgs genArgs)

    let rec makeType (com: Compiler) (ctxTypeArgs: Map<string, Fable.Type>) (NonAbbreviatedType t) =
        // Generic parameter (try to resolve for inline functions)
        if t.IsGenericParameter then
            resolveGenParam ctxTypeArgs t.GenericParameter
        // Tuple
        elif t.IsTupleType then
            makeGenArgs com ctxTypeArgs t.GenericArguments
            |> Fable.Tuple
        // Function
        elif t.IsFunctionType then
            let argType =
                makeType com ctxTypeArgs t.GenericArguments.[0]

            let returnType =
                makeType com ctxTypeArgs t.GenericArguments.[1]
    ...
    let getBaseClass (tdef: FSharpEntity) =
        ...
        | Some (TypeDefinition tdef) when tdef.TryFullName <> Some Types.object -> Some tdef
        | _ -> None

    let rec getOwnAndInheritedMembers (com: Compiler) (ent: Fable.Entity) =
        seq {
            yield! ent.MembersFunctionsAndValues
            match ent.BaseDeclaration with
            | Some baseEnt ->
                yield! com.GetEntity(baseEnt.Definition)
                       |> getOwnAndInheritedMembers com
            | _ -> ()
        }

    let getArgTypes (memb: Fable.MemberFunctionOrValue) =
    ...
    let tryGetInterfaceTypeFromMethod (meth: FSharpMemberOrFunctionOrValue) =
        if meth.ImplementedAbstractSignatures.Count > 0 then
            nonAbbreviatedType meth.ImplementedAbstractSignatures.[0].DeclaringType
            |> Some
        else
            None
    ...
    let tryFindMember com (entity: Fable.Entity) genArgs compiledName isStatic (argTypes: Fable.Type list) =
        let argsEqual (args1: Fable.Type list) args1Length (args2: Fable.Parameter list list) =
            let args2Length = args2 |> Seq.sumBy List.length
            if args1Length = args2Length then
                let args2 =
                    args2
                    |> Seq.collect (fun g ->
                        // TODO: We may need to resolve the gen arg here
                        g |> Seq.map (fun p -> p.Type) |> Seq.toList)
        ...
        getOwnAndInheritedMembers com entity
        |> Seq.tryFind (fun m2 ->
            if m2.Info.IsStatic = isStatic
               && m2.CompiledName = compiledName then
                argsEqual argTypes argTypesLength m2.CurriedParameterGroups
            else
                false)
    ...
    let fitsAnonRecordInInterface com (argExprs: Fable.Expr list) fieldNames (interface_: Fable.Entity) =
        ...
        (Ok(),
         getAllInterfaceMembers com interface_
         |> Seq.filter (fun memb -> memb.Info.IsGetter))
        ||> Seq.fold (fun res memb ->
                match res with
                | Error _ -> res
                | Ok _ ->
                    let expectedType = memb.ReturnParameter.Type
                    List.tryFindIndex ((=) memb.DisplayName) fieldNames
                    |> function
                    | None ->
                        match expectedType with
                        | Fable.Option _ -> Ok() // Optional fields can be missing
                        | _ ->
                            sprintf "Object doesn't contain field '%s'" memb.DisplayName
                            |> Error
                    | Some i ->
                        ...
                        | false ->
                            ...
                            sprintf "Expecting type '%s' for field '%s'" typeName memb.DisplayName
                            |> Error)
...
module Identifiers =
    ...
    let putIdentInScope (ctx: Context) (fsRef: FSharpMemberOrFunctionOrValue) (ident: Fable.Ident) value =
        { ctx with
              Scope = (fsRef, ident, value) :: ctx.Scope }
    ...
    let makeIdentFrom (com: IFableCompiler) (ctx: Context) (fsRef: FSharpMemberOrFunctionOrValue): Fable.Ident =
        ...
        ctx.UseNamesInDeclarationScope.Add(sanitizedName)
        |> ignore
        { Name = sanitizedName
          ...
          Range =
              { makeRange fsRef.DeclarationLocation with
                    identifierName = Some fsRef.DisplayName }
              |> Some }
    ...
    let identWithRange r (ident: Fable.Ident) =
        ...
        { ident with
              Range =
                  r
                  |> Option.map (fun r -> { r with identifierName = originalName }) }

    let tryGetIdentFromScopeIf (ctx: Context) r predicate =
        ctx.Scope
        |> List.tryPick (fun (fsRef, ident, _) ->
            if predicate fsRef
            then identWithRange r ident |> Fable.IdentExpr |> Some
            else None)
    ...
    let rec tryGetBoundValueFromScope (ctx: Context) identName =
        match ctx.Scope
              |> List.tryFind (fun (_, ident, _) -> ident.Name = identName) with
        | Some (_, _, value) ->
            match value with
            | Some (Fable.IdentExpr ident) when not ident.IsMutable -> tryGetBoundValueFromScope ctx ident.Name
            | v -> v
        | None -> None
...
module Util =
    ...
    let countNonCurriedParams (meth: Fable.MemberFunctionOrValue) =
        match meth.CurriedParameterGroups with
        | [] -> 0
        | [p]::_ ->
            match p.Type with
            | Fable.Unit -> 0
            | _ -> 1
        | nonCurried::_ -> List.length nonCurried
    ...
    let countNonCurriedParamsForSignature (sign: FSharpAbstractSignature) =
        ...
        elif args.[0].Count = 1 then if isUnit args.[0].[0].Type then 0 else 1
        else args.[0].Count
    ...
    let fixImportedRelativePath (com: Compiler) (path: string) normalizedSourcePath =
        if Path.isRelativePath path then
            let file =
                Path.normalizePathAndEnsureFsExtension normalizedSourcePath
    ...
    let (|GlobalAtt|ImportAtt|NoGlobalNorImport|) (atts: Fable.Attribute list) =
        let (|AttFullName|) (att: Fable.Attribute) = att.FullName, att

        atts |> List.tryPick (function
    ...
    let tryGlobalOrImportedMember (com: Compiler) typ (memb: Fable.MemberFunctionOrValue) =
        match memb.Attributes with
        | GlobalAtt (Some customName) ->
            makeTypedIdent typ customName
            |> Fable.IdentExpr
            |> Some
        | GlobalAtt None ->
        ...
        | ImportAtt (selector, path) ->
            let selector =
                if selector = Naming.placeholder then getMemberDisplayName memb else selector

            let path =
                match memb.DeclaringEntity with
                | Some e -> com.GetEntity(e).SourcePath |> fixImportedRelativePath com path
                | None -> path

            makeImportUserGenerated None typ (makeStrConst selector) (makeStrConst path)
            |> Some
        | _ -> None
    ...
    let tryGlobalOrImportedEntity (com: Compiler) (ent: Fable.Entity) =
        ...
        | GlobalAtt (Some customName) ->
            makeTypedIdent Fable.Any customName
            |> Fable.IdentExpr
            |> Some
        | GlobalAtt None ->
            ent.DisplayName
        ...
        | ImportAtt (selector, path) ->
            let selector =
                if selector = Naming.placeholder then ent.DisplayName else selector

            fixImportedRelativePath com path ent.SourcePath
            |> makeStrConst
            |> makeImportUserGenerated None Fable.Any (makeStrConst selector)
            |> Some
        | _ -> None
    ...
    let entityRefWithSuffix (com: Compiler) (ent: Fable.Entity) suffix =
        ...
        if ent.IsInterface then
            ...
            let entityName =
                getEntityDeclarationName com ent + suffix

            if file = com.CurrentFile
            then makeIdentExpr entityName
            elif ent.IsPublic
            then makeImportInternal com Fable.Any entityName file
            else error "Cannot inline functions that reference private entities"

    let entityRef (com: Compiler) (ent: Fable.Entity) = entityRefWithSuffix com ent ""
    ...
    let memberRefTyped (com: Compiler) (ctx: Context) r typ (memb: FSharpMemberOrFunctionOrValue) =
        let r =
            ...
            |> Option.map (fun r ->
                { r with
                      identifierName = Some memb.DisplayName })
    ...
    let isMangledAbstractEntity (ent: Fable.Entity) =
        ...
        if ent.FullName.StartsWith("System.") then
            match ent.FullName with
            | Types.object
            | Types.idisposable
            | Types.icomparable
        ...
        elif ent.IsInterface then hasAttribute Atts.mangle ent.Attributes
        // Mangle members from abstract classes unless they are global/imported
        else not (isGlobalOrImportedEntity ent)
    ...
    let callInstanceMember com
        ...
        let isSetter = not isGetter && memb.Info.IsSetter

        let indexedProp =
            (isGetter && countNonCurriedParams memb > 0)
            || (isSetter && countNonCurriedParams memb > 1)

        let name, isGetter, isSetter =
            if isMangledAbstractEntity entity then
                let overloadHash =
                    if (isGetter || isSetter) && not indexedProp
                    then ""
    ...
    let (|Replaced|_|) (com: IFableCompiler)
                       ctx
                       r
                       typ
                       (genArgs: Lazy<_>)
                       (callInfo: Fable.CallInfo)
                       (memb: Fable.MemberFunctionOrValue, entity: Fable.Entity option)
                       =
        match entity with
        | Some ent when isReplacementCandidate ent.FullName ->
            ...
            | None when info.IsInterface ->
                callInstanceMember com r typ callInfo ent memb
                |> Some
            | None ->
    ...
    let (|Emitted|_|) com r typ (callInfo: Fable.CallInfo option) (memb: Fable.MemberFunctionOrValue) =
        memb.Attributes
        |> List.tryPick (fun att ->
            if att.FullName.StartsWith Atts.emit then
                let callInfo =
                    match callInfo with
                    | Some i -> i
                    | None ->
                ...
                let callInfo =
                    ...
                    | Some importExpr ->
                        { callInfo with
                              Fable.ThisArg = Some importExpr }
                    | _ -> callInfo

                let isStatement =
                    tryAttributeConsArg att 1 false tryBoolean
    ...
    let (|Imported|_|) com r typ callInfo (memb: Fable.MemberFunctionOrValue, entity: Fable.Entity option) =
        let importValueType =
            if Option.isSome callInfo then Fable.Any else typ
        ...
        | None, Some callInfo, Some e ->
            ...
            | Some _, Some _thisArg ->
                callInstanceMember com r typ callInfo e memb
                |> Some

            | Some classExpr, None when memb.IsConstructor ->
                Fable.Call(classExpr, { callInfo with IsJsConstructor = true }, typ, r)
                |> Some
            ...
            | Some moduleOrClassExpr, None ->
                if isModuleValueForCalls e memb then
                    ...
                    let fieldGet =
                        makeFieldKey (getMemberDisplayName memb) true Fable.Any

                    Fable.Get(moduleOrClassExpr, Fable.ByKey fieldGet, typ, r)
                    |> Some
                else
                    let callInfo =
                        { callInfo with
                              ThisArg = Some moduleOrClassExpr }
    ...
    let inlineExpr (com: IFableCompiler)
                   (ctx: Context)
                   r
                   (genArgs: Lazy<_>)
                   callee
        ...
        let rec foldArgs acc =
            ...
            | (argIdent: FSharpMemberOrFunctionOrValue) :: restArgIdents, [] ->
                let t =
                    makeType com ctx.GenericArgs argIdent.FullType

                foldArgs
                    ((argIdent, Fable.Value(Fable.NewOption(None, t), None))
                     :: acc)
                    (restArgIdents, [])
            | [], _ -> List.rev acc
        ...
        | _ ->
            ...
            let ctx, bindings =
                ...
                ||> List.fold (fun (ctx, bindings) (argId, arg) ->
                        ...
                        let ident =
                            { makeIdentFrom com ctx argId with
                                  Type = arg.Type
                                  IsCompilerGenerated = true }

                        let ctx =
                            putIdentInScope ctx argId ident (Some arg)
    ...
    let (|Inlined|_|) (com: IFableCompiler) ctx r genArgs callee args (memb: FSharpMemberOrFunctionOrValue) =
        if not (isInline memb) then
            None
        else
            inlineExpr com ctx r genArgs callee args memb
            |> Some

    /// Removes optional arguments set to None in tail position and calls the injector if necessary
    let transformOptionalArguments (com: IFableCompiler)
                                   (ctx: Context)
                                   r
                                   (memb: Fable.MemberFunctionOrValue)
                                   (genArgs: Lazy<_>)
                                   ...
                                   =
        if memb.CurriedParameterGroups.Length
           <> 1
           || memb.CurriedParameterGroups.[0].Length
            ...
            |||> Seq.foldBack2 (fun par arg (condition, acc) ->
                     ...
                     | "optional"
                     | "inject" when par.IsOptional ->
                         match arg with
                         | Fable.Value (Fable.NewOption (None, _), _) ->
                             if par.Attributes
                                |> List.exists (fun a -> a.FullName = Atts.inject) then
                                 "inject",
                                 (com.InjectArgument(ctx, r, genArgs.Value, par))
                                 :: acc
                             else
                                 // Don't remove optional arguments if they're not in tail position
                                 condition, (if condition = "optional" then acc else arg :: acc)
    ...
    let hasInterface interfaceFullname (ent: Fable.Entity) =
        ent.AllInterfaces
        |> Seq.exists (fun ifc -> ifc.Definition = interfaceFullname)

    let makeCallWithArgInfo com
                            ctx
                            r
                            typ
                            genArgs
        ...
        | Try (tryGetIdentFromScope ctx r) funcExpr, Some entity ->
            if isModuleValueForCalls entity memb then funcExpr else makeCall r typ callInfo funcExpr
        ...
        | _, Some entity when entity.IsInterface
                              ...
                              || memb.IsDispatchSlot -> callInstanceMember com r typ callInfo entity memb

        | _, Some entity when isModuleValueForCalls entity memb ->
            let typ =
                makeType com ctx.GenericArgs memb.FullType
    ...
    let makeCallInfoFrom (com: IFableCompiler)
                         ctx
                         r
                         genArgs
                         callee
        ...
        { ThisArg = callee
          ...
          SignatureArgTypes = getArgTypes memb
          HasSpread = hasParamArray memb
          IsJsConstructor = false }

    let makeCallFrom (com: IFableCompiler)
                     (ctx: Context)
                     r
                     typ
                     (genArgs: Fable.Type seq)
                     callee
                     args
                     (memb: Fable.MemberFunctionOrValue)
                     =
        let genArgs =
            lazy (matchGenericParamsFrom memb genArgs |> Seq.toList)

        makeCallInfoFrom com ctx r genArgs callee args memb
        |> makeCallWithArgInfo com ctx r typ genArgs callee memb

    let makeValueFrom (com: IFableCompiler) (ctx: Context) r (v: Fable.MemberFunctionOrValue) =
        let typ = v.FullType
        let e = v.DeclaringEntity |> Option.map com.GetEntity
        match v, e with
        | _ when typ = Fable.Unit ->
            if com.Options.Verbosity = Verbosity.Verbose
               && not v.IsCompilerGenerated then // See #1516
